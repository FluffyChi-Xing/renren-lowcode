  ...
  return new Promise<string>((resolve, reject) => {
    try {
      const debounceInsert = debounce(() => {
        if ($util.renren.isElementAMaterialModelType(mySchemaStore.newElement)) {
          resetStack.push(mySchemaStore.newElement as RenrenMaterialModel);
          resolve('插入物料事件处理成功');
        }
      }, 100);
      debounceInsert();
  ...
  return new Promise<T>((resolve, reject) => {
    try {
      ...
      if ($util.renren.isElementAMaterialModelType(head)) {
        ...
        // sync to store
        mySchemaStore.elementInProcess = undefined; // 初始化 store
        mySchemaStore.elementInProcess = head as RenrenMaterialModel;
        resolve(head as T);
      }
    } catch (e) {
  ...
  return new Promise<T>((resolve, reject) =>{
    try {
      ...
      if ($util.renren.isElementAMaterialModelType(head)) {
        ...
        // sync to store
        mySchemaStore.elementInProcess = undefined;
        mySchemaStore.elementInProcess = head as RenrenMaterialModel;
        resolve(head as T);
      }
    } catch (e) {
 ...
 */
watch(() => mySchemaStore.newElement, () => {
  const material = mySchemaStore.newElement as RenrenMaterialModel;
 ...
 */
$event.on(`addAnimation:${(mySchemaStore.currentElement as RenrenMaterialModel)?.id}`, () => {
 ...
 */
$event.on(`updateMaterial:${(mySchemaStore.currentElement as RenrenMaterialModel)?.id}`, () => {
  updateTargetMaterial().catch(err => {
    console.log(err as string);
  });
});
